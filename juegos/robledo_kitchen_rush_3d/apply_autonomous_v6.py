...
# Waiter ownership: Service Captain gets first refusal when one is autonomous.
...
# Physical-navigation correction discovered by browser QA:
# walking toward the CENTER of a rectangular fixture can collide with the inflated AABB at a
# corner before the old radial interaction threshold is reached. Bots now choose a reachable
# cardinal service point just outside the actual collision box, preserving the same real
# fixture/interact state machine while eliminating corner deadlocks at tables and stations.
...
if old_near not in text:
    raise SystemExit('Could not apply V6 collision-aware approach navigation correction')
