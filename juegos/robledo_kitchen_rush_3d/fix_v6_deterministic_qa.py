...
pattern = re.compile(r"  stepPlaying\(dt\)\{.*?requestAnimationFrame\(t=>this\.loop\(t\)\);\}\n", re.S)
text, count = pattern.subn(production_loop + '\n', text, count=1)
if count != 1:
    raise SystemExit(f'Expected to restore one V6 loop region, replaced {count}')
...
// Drive the exact production subsystems with a fixed 50 ms simulation clock. During this phase
// the RAF loop is disabled by changing only the top-level state to `qa`; BotBrain, Party,
// collision, WorldItem, stations, cooking and table interaction code remain production code.
async function advance(cdp, seconds){const total=Math.ceil(seconds/.05);for(let done=0;done<total;done+=20){const n=Math.min(20,total-done);await cdp.eval(`(()=>{const g=window.__rkrGame;for(let j=0;j<${n};j++){for(const p of g.players)p.update(.05);g.updateStations(.05);g.updateThrown(.05);for(const p of g.parties)p.update(.05);g.parties=g.parties.filter(p=>p.state!=='gone');g.updateUI();}return true;})()`);}}
async function advanceUntil(cdp, expression, maxSeconds=60){for(let t=0;t<maxSeconds;t++){await advance(cdp,1);if(await cdp.eval(expression))return t+1;}const snap=await cdp.eval(`(()=>{const g=window.__rkrGame;return{state:g.state,lives:g.lives,parties:g.parties.map(p=>({state:p.state,claim:p.orderClaim,taken:p.orderTakenBy,wait:+p.waitElapsed.toFixed(2),orders:p.orders.map(o=>({r:o.recipeId,c:o.claimedBy})),table:p.table?.id+1})),bots:g.players.filter(p=>!p.human).map(p=>({i:p.index,role:p.role,pos:{x:+p.group.position.x.toFixed(2),z:+p.group.position.z.toFixed(2)},held:p.held?.description?.()||null,task:p.bot?.task?.kind||null,recipe:p.bot?.task?.recipeId||null,action:p.bot?.task?.actions?.[p.bot?.actionIndex]?.kind||null,actionIndex:p.bot?.actionIndex||0,process:p.bot?.processStation?.type||null})),stations:g.stations.filter(s=>s.type!=='storage').map(s=>({type:s.type,pos:{x:s.pos.x,z:s.pos.z},reserved:s.reservedBy||null,slot:s.slot?.description?.()||null,state:s.slot?.state||null,progress:+(s.progress||0).toFixed(2),cook:+(s.cook||0).toFixed(2),ready:!!s.ready})),events:g.qaEvents.slice(-50)};})()`);throw new Error(`Simulated ${maxSeconds}s without satisfying ${expression}; snapshot=${JSON.stringify(snap)}`);}
...
const started=await cdp.eval(`(()=>{const g=window.__rkrGame;g.state='qa';g.spawnTimer=9999;g.timeLeft=9999;return{hud:getComputedStyle(document.getElementById('hud')).display!=='none',crew:g.players.length,bots:g.players.filter(p=>!p.human).length,roles:g.players.map(p=>p.role),storage:g.stations.filter(s=>s.type==='storage').map(s=>s.kind).sort(),timer:document.getElementById('timer').textContent};})()`);
...
// A — Service Captain takes order, then a bot completes a physical salad chain.
...
// B — Burger proves real grocery pickup and stove state transitions.
await cdp.eval(`(()=>{const g=window.__rkrGame;const marker=g.qaEvents.length;g.spawnParty();const p=g.parties[g.parties.length-1];p.state='readyToOrder';p.browse=0;p.orderWaitElapsed=4;p.waitElapsed=0;p.patience=180;p.patienceMax=180;p.orders=[{id:'qa-burger',recipeId:'burger',claimedBy:null}];p.originalRecipes=['burger'];p.size=1;p.orderClaim=null;p.orderTakenBy=null;p.customers.forEach((c,i)=>{c.group.position.copy(p.table.seats[i]);c.group.position.y=-.08;});g.__qaMarker=marker;return true;})()`);
...
// C — Dirty plate must physically go from table to sink to clean plate rack.
...
print('Applied deterministic production-state V6 browser QA harness.')
