...
async function keyTicks(cdp,code,keyValue,playerIndex,ticks=6,dt=.05){await cdp.send('Input.dispatchKeyEvent',{type:'keyDown',code,key:keyValue});await cdp.eval(`(()=>{const g=window.__rkrGame,p=g.players[${playerIndex}];for(let i=0;i<${ticks};i++)p.update(${dt});return true;})()`);await cdp.send('Input.dispatchKeyEvent',{type:'keyUp',code,key:keyValue});await cdp.eval(`window.__rkrGame.players[${playerIndex}].update(${dt})`);}
async function mouseClick(cdp,x,y,button='left'){await cdp.send('Input.dispatchMouseEvent',{type:'mouseMoved',x,y});await cdp.send('Input.dispatchMouseEvent',{type:'mousePressed',x,y,button,clickCount:1});await cdp.send('Input.dispatchMouseEvent',{type:'mouseReleased',x,y,button,clickCount:1});}
...
const setup=await cdp.eval(`(()=>{const g=window.__rkrGame;g.config.humanCount=3;g.config.roles=['chef','prep','service'];g.config.menu=['salad','burger'];g.business.cash=430;g.business.satisfaction=80;g.business.plan=[];g.plan=[];g.enterBuildMode();g.autoLayout();g.openRestaurant();g.spawnTimer=9999;g.timeLeft=9999;const ps=g.players;ps[0].group.position.set(-3.1,0,.15);ps[1].group.position.set(0,0,.15);ps[2].group.position.set(3.1,0,.15);g.state='qa-controls';return{v:g.__controlsV7.version,humans:g.players.filter(p=>p.human).length,roles:g.players.map(p=>p.role),plan:g.plan.map(q=>q.key),controls:[...document.querySelectorAll('#controls-screen article')].slice(0,3).map(x=>x.textContent)};})()`);
...
await keyTicks(cdp,'KeyF','f',1,1,.05);
...
await keyTicks(cdp,'KeyE','e',1,10,.05);
...
await keyTicks(cdp,'Period','.',2,1,.05);
...
await keyTicks(cdp,'Enter','Enter',2,4,.05);
...
await cdp.eval(`(()=>{const g=window.__rkrGame;const ps=g.players;ps[0].group.position.set(-3.1,0,.15);ps[1].group.position.set(0,0,.15);ps[2].group.position.set(3.1,0,.15);g.__controlsV7.selectTarget(ps[0],null);g.__controlsV7.mouse.moveTarget=null;g.cameraRig.setMode('classic');g.state='qa-controls';return true;})()`);
await sleep(120);
...
await cdp.eval(`window.__rkrGame.state='playing'`);
await mouseClick(cdp,floorPoint.x,floorPoint.y,'left');
await cdp.eval(`(()=>{const g=window.__rkrGame;g.state='qa-controls';const p=g.players[0];for(let i=0;i<14;i++)p.update(.05);return true;})()`);
...
if(!(p1Moved.z>floorPoint.z0+.45))throw new Error(`P1 left-click movement failed: ${JSON.stringify({floorPoint,p1Moved})}`);

// P1 left click on a fixture selects it; right click/hold interacts without camera conflict.
const p1PlatePoint=await cdp.eval(`(()=>{const g=window.__rkrGame,p=g.players[0],s=g.stations.find(x=>x.type==='plate');p.group.position.set(s.pos.x,0,s.pos.z+1.25);p.prevInteract=false;g.__controlsV7.selectTarget(p,null);g.state='qa-controls';const v=s.pos.clone();v.y=1.0;v.project(g.camera);const r=g.canvas.getBoundingClientRect();return{x:r.left+(v.x+1)*r.width/2,y:r.top+(1-v.y)*r.height/2,yaw:g.cameraRig.yaw};})()`);
await cdp.eval(`window.__rkrGame.state='playing'`);
await mouseClick(cdp,p1PlatePoint.x,p1PlatePoint.y,'left');
await cdp.eval(`window.__rkrGame.state='qa-controls'`);
...
await cdp.eval(`(()=>{const g=window.__rkrGame;g.state='qa-controls';const p=g.players[0];for(let i=0;i<10;i++)p.update(.05);return true;})()`);
await cdp.eval(`window.__rkrGame.state='playing'`);
await cdp.send('Input.dispatchMouseEvent',{type:'mouseReleased',x:p1PlatePoint.x,y:p1PlatePoint.y,button:'right',clickCount:1});
await cdp.eval(`(()=>{const g=window.__rkrGame;g.state='qa-controls';g.players[0].update(.05);return true;})()`);
