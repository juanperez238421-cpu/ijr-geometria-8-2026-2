...
// Explicit navigation avoids occasionally attaching to Chromium's transient startup document,
// which has an opaque origin and correctly rejects localStorage access.
...
const prepPoint=await cdp.eval(`(()=>{const g=window.__rkrGame,p=g.players[0],s=g.stations.find(x=>x.type==='prep');p.group.position.set(s.pos.x,0,s.pos.z+1.38);g.cameraRig.update(.5);const v=s.pos.clone();v.y=1.05;v.project(g.camera);const r=g.canvas.getBoundingClientRect();return{x:r.left+(v.x+1)*r.width/2,y:r.top+(1-v.y)*r.height/2};})()`);
await mouseClick(cdp,prepPoint.x,prepPoint.y,'left',90);await waitEval(cdp,`window.__rkrGame.stations.find(x=>x.type==='prep')?.slot?.state==='chopped'`,5000);
...
const board=await cdp.eval(`(()=>{const g=window.__rkrGame;g.__seniorV8.cleanliness();g.updateUI();const el=document.getElementById('v8-service-board');return{text:el.textContent,stock:Object.keys(g.v8Stock).length,clean:g.__seniorV8.cleanliness()};})()`);
